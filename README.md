...
- 记录路由和时间戳选项
- 无需管理员权限
- **TCP 端口开放检测**（支持单端口、多端口、端口范围）
- 常见服务识别（HTTP、HTTPS、SSH、RDP、SMB 等）
- **中英文界面切换**（支持 `-ZH`、`-EN` 选项）
...
# 端口检测：扫描常用端口（HTTP、SSH、MySQL 等）
qping 192.168.0.1 -p

# 端口检测：检测单个端口
qping 192.168.0.1 -p 80

# 端口检测：检测多个端口
qping 192.168.0.1 -p 80,443

# 端口检测：检测端口范围
qping 192.168.0.1 -p 1-1024

# 端口检测：扫描网段的常用端口
qping 192.168.1.0/24 -p 22,80,443

# 端口检测：批量 IP + 端口
qping 192.168.0.1-10 -p 135,445
...
| 选项 | 说明 |
|------|------|
| `-p [ports]` | 检测端口，不指定端口时扫描常用端口 |

**端口格式说明：**

| 格式 | 示例 | 说明 |
|------|------|------|
| 默认扫描 | `-p` | 扫描常用端口（21个，包括 HTTP、SSH、MySQL 等） |
| 指定范围 | `-p 1-1024` | 扫描 1 到 1024 端口 |
| 单端口 | `-p 80` | 检测 80 端口 |
| 多端口 | `-p 80,443,3389` | 检测 80、443、3389 端口 |
| 端口范围 | `-p 1-1024` | 检测 1 到 1024 端口 |
| 混合 | `-p 80,443,8080-8090` | 检测 80、443 以及 8080 到 8090 端口 |
...
总目标数: 4
端口检测模式: 2 个端口
127.0.0.1:135 开放 () 时间=1ms
127.0.0.1:445 开放 (SMB) 时间=16ms
127.0.0.1:80 关闭
127.0.0.1:443 关闭

--- 统计信息 ---
127.0.0.1:135 : 已发送=1, 已接收=1, 丢失=0 (0.0%)
127.0.0.1:445 : 已发送=1, 已接收=1, 丢失=0 (0.0%)
127.0.0.1:80 : 已发送=1, 已接收=0, 丢失=1 (100.0%)
127.0.0.1:443 : 已发送=1, 已接收=0, 丢失=1 (100.0%)

端口统计: 发送=4, 开放=2, 关闭=2 (50.0%)
...
- **新增中英文界面切换功能**
  ...
  - 语言设置自动保存到注册表，永久生效
  - 所有帮助信息、统计信息均支持中英文切换
- 优化语言加载顺序，确保帮助信息在语言设置之后显示

### v1.3.0

- **新增 TCP 端口开放检测功能**
  - 使用 `-p` 选项指定端口（单端口、多端口、端口范围）
  - 支持格式：`-p 80`、`-p 80,443`、`-p 1-1024`
  - `-p` 不带参数时扫描 23 个常用端口（HTTP、SSH、MySQL、RDP 等）
  - 自动识别常见服务（HTTP、HTTPS、SSH、RDP、SMB、MySQL 等）
  - 端口检测支持与 IP 范围、CIDR 组合使用
- 修复 `-p` 参数被错误识别为域名的问题
