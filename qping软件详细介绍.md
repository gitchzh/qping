...
总目标数: 2
端口检测模式: 2 个端口
127.0.0.1:445 开放 (SMB) 时间=0ms
127.0.0.1:3389 开放 (RDP) 时间=0ms

--- 统计信息 ---
127.0.0.1:445 : 已发送=1, 已接收=1, 丢失=0 (0.0%)
127.0.0.1:3389 : 已发送=1, 已接收=1, 丢失=0 (0.0%)

端口统计: 发送=2, 开放=2, 关闭=0 (0.0%)
...
# 端口检测
qping 192.168.1.1 -p              # 扫描常用端口（HTTP、SSH、MySQL 等）
qping 192.168.1.1 -p 80           # 检测单个端口
qping 192.168.1.1 -p 80,443       # 检测多个端口
qping 192.168.1.1 -p 1-1024       # 检测端口范围
qping 192.168.1.0/24 -p 22,80,443 # 扫描网段的常用端口
...
### 用户友好
- **中文支持**：完美支持中文系统，无乱码问题
- **中英文界面切换**：使用 `-ZH` 和 `-EN` 切换语言，设置自动保存
...
*作者：mrchzh | 邮箱：gmrchzh@gmail.com | 版本：v1.4.0*